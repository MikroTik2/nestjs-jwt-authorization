...
export class StorageService {
	...
	public async uploadFile(file: Express.Multer.File, buffer: Buffer, tag: string) {
		...
		try {
			...
			this.logger.error(`❌ Failed when uploading file. Error: ${error.message}`, error.stack)
			throw new Error(`❌ Failed when uploading file. Error: ${error.message}`)
		}
	}
	...
	public async deleteFile(tag: string, id: string) {
		...
		try {
			...
			this.logger.error(`❌ Failed when deleting file. Error: ${error.message}`, error.stack)
			throw new Error(`❌ Failed when deleting file. Error: ${error.message}`)
		}
	}
}
